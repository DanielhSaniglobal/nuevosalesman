...
async function cargarZonas() {
  ...
  // Ordenar por longitud del nombre de la colonia (descendente)
  // Esto evita que colonias con nombres cortos (ej. 'Real') hagan match antes que nombres más específicos (ej. 'Valle Real')
  allZonas.sort((a, b) => {
    const lenA = a.colonia ? a.colonia.length : 0;
    const lenB = b.colonia ? b.colonia.length : 0;
    return lenB - lenA; // de mayor a menor
  });
...
async function buscarTier(mensaje) {
  ...
  try {
    ...
    // Contexto Geográfico: Detectar si mencionan un estado/municipio específico
    const esSLP = /\b(slp|san luis|potosi|potosí)\b/i.test(mensajeMinc);
    const esJalisco = /\b(jalisco|jal|gdl|guadalajara|zapopan|tlaquepaque|tonala|tonalá|tlajomulco|salto)\b/i.test(mensajeMinc);

    // Creamos una lista ordenada específicamente para este mensaje priorizando la ubicación correcta
    let zonasPriorizadas = [...zonas];
    zonasPriorizadas.sort((a, b) => {
      let scoreA = 0;
      let scoreB = 0;

      // Si el mensaje indica SLP, las zonas de SLP (tier 6) tienen máxima prioridad
      if (esSLP && !esJalisco) {
        if (a.municipio_id === 6) scoreA += 10000;
        if (b.municipio_id === 6) scoreB += 10000;
      } 
      // Si indica Jalisco, o si no indica nada (por defecto Jalisco es la matriz principal)
      else if (esJalisco || (!esJalisco && !esSLP)) {
        if (a.municipio_id !== 6) scoreA += 10000;
        if (b.municipio_id !== 6) scoreB += 10000;
      }

      // Desempate por longitud (como ya teníamos en caché, pero lo aseguramos aquí)
      const lenA = a.colonia ? a.colonia.length : 0;
      const lenB = b.colonia ? b.colonia.length : 0;
    ...
    if (cpsEnMensaje.length > 0) {
      // Buscar zonas que coincidan con los CP encontrados en el array priorizado
      const zonasPorCP = zonasPriorizadas.filter(z => z.codigo_postal && cpsEnMensaje.includes(z.codigo_postal));
      ...
      if (zonasPorCP.length > 0) {
        ...
        // Si no se cercioró ninguna colonia, devolvemos la primera del CP ordenado por prioridad
        return zonasPorCP[0];
      }
    }
    ...
    for (const zona of zonasPriorizadas) {
      if (zona.colonia) {
        const nombreColoniaEscapado = zona.colonia.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
